...
def handle_interpolation(req):
    ...
    while k<=kkonc:
        ...
        x_val=xx.interpolation(req.type, req.px , req.x,kkonc,k)
        y_val=yy.interpolation(req.type, req.py, req.y,kkonc,k)
        z_val=zz.interpolation(req.type, req.pz, req.z,kkonc,k)
        roll_val=rolll.interpolation(req.type, 0, req.roll,kkonc,k)
        pitch_val=pitchh.interpolation(req.type, 0, req.pitch,kkonc,k)
        yaw_val=yaww.interpolation(req.type, 0, req.yaw,kkonc,k)
